...
const OverViewBox = ({ pillsRef, expand, setExpand }: any) => {
  ...
  return (
    <div className={`p-[0.8vh] box ${expand ? 'h-full' : 'h-[30%]'} `} id="takeover">
      <div
        ...
        <div className={`flex flex-col items-center gap-[2vh] px-6 py-8 text-box group `}>
          ...
          <div className="flex flex-col gap-[1.5vh]">
            {expand ? (
              <>
                {' '}
                <p className={`text-[#1e1e1e] font-bold ${mobileView ? 'text-[1.5vh]' : 'text-md xl:text-lg'}`}>
                ...
                <p className={` ${mobileView ? 'text-[1.5vh]' : 'text-md xl:text-lg'} text-[#1e1e1e] font-bold`}>
          ...
          {!expand ? (
            ...
            <button className="flex flex-col items-center mt-auto lg:flex-row intro" onClick={() => setExpand(!expand)}>
              <p className={`${fontColor}`}>See Less</p>
            </button>
          )}
        </div>
      </div>
